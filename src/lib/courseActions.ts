...
// ─── Admin force-delete any course (including APPROVED) ──────────────────────
...
export const adminDeleteCourse = async (currentState: State, data: FormData) => {
  ...
  try {
    ...
    // Get username of the admin deleting the course
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const adminExpireCourse = async (currentState: State, data: FormData) => {
  ...
  try {
    ...
    // Allow admins to expire any course, or teachers to expire their own courses
    ...
    // Get username of the user marking the course as expired
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const unexpireCourse = async (currentState: State, data: FormData) => {
  ...
  try {
    ...
    // Get username of the user unexpiring the course
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateCourse = async (currentState: State, data: FormData) => {
  ...
  try {
    ...
    if (before) {
      ...
      // Get username of the user updating the course
      const client = await clerkClient();
      const user = await client.users.getUser(userId || "");
      const username = user.username || "unknown";
...
export const deleteCourse = async (currentState: State, data: FormData) => {
  ...
  try {
    ...
    // Allow admins to delete any course, or teachers to delete their own courses
    if (role !== "admin" && course.teacherId !== userId) {
      return { success: false, error: true };
    }
    ...
    // Get username of the user deleting the course
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const markLectureComplete = async (currentState: State, data: FormData) => {
  ...
  const completed  = data.get("completed") === "true"; // current persisted state
  try {
