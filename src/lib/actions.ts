...
export const createClass = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user creating the class
...
export const updateClass = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the class
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteClass = async (
  ...
  const id = data.get("id") as string;
  try {
    // Get class data before deletion for specific notification
...
export const deleteTeacher = async (
  ...
  const id = data.get("id") as string;
  try {
    // Get teacher data before deletion for notification
...
export const updateExam = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the exam
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteExam = async (
  ...
  try {
    // Get exam data before deletion for specific notification
...
export const createEvent = async (
  ...
  try {
    ...
    // Get username of the user creating the event
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateEvent = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the event
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteEvent = async (
  ...
  try {
    // Get event data before deletion for specific notification
...
export const createAnnouncement = async (
  ...
  try {
    ...
    // Handle classId - remove if empty string, 0, or convert to number
    if (announcementData.classId !== undefined && announcementData.classId !== null) {
      ...
      // Validate classId if still provided after cleanup
      if (announcementData.classId !== undefined) {
        ...
        if (!classExists) {
          console.log("Class not found, removing classId from announcement data");
          delete announcementData.classId;
        }
      }
    }
    ...
    // Get username of the user creating the announcement
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateAnnouncement = async (
  ...
  try {
      ...
      if (typeof announcementData.classId === 'string') {
        if (announcementData.classId === '' || announcementData.classId === '0') {
          ...
          if (isNaN(parsedId) || parsedId === 0) {
            ...
            // Validate classId if it's a valid number
            const classExists = await prisma.class.findUnique({
              where: { id: parsedId },
              select: { id: true },
            });
    ...
    // Get username of the user updating the announcement
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteAnnouncement = async (
  ...
  try {
    // Get announcement data before deletion for specific notification
...
export const createLesson = async (
  ...
  try {
    ...
    // Get username of the user creating the lesson
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateLesson = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the lesson
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteLesson = async (
  ...
  const id = data.get("id") as string;
  try {
    // Get lesson data before deletion for specific notification
    const lesson = await prisma.lesson.findUnique({
      where: { id: parseInt(id) },
      select: {
        name: true,
...
export const createAssignment = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user creating the assignment
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateAssignment = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the assignment
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const deleteAssignment = async (
  ...
  const id = data.get("id") as string;
  try {
    // Get assignment data before deletion for specific notification
...
export const createResult = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user posting the result
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
...
export const updateResult = async (
  ...
  const { userId } = auth();
  try {
    // Get username of the user updating the result
    const client = await clerkClient();
    const user = await client.users.getUser(userId || "");
    const username = user.username || "unknown";
