...
import { UpdateDetailsDialog } from "@/components/notifications/UpdateDetailsDialog";
...
/* ── Arrow that reacts to the folder's open/close state ─────────────────── */
...
const menuGroups: MenuGroup[] = [
  {
    ...
    icon: Home,
    items: [
      { icon: Home, label: "Dashboard", href: "/", visible: ["admin", "teacher", "student", "parent"] },
...
function NavContent({
  ...
  const getBadgeInfo = (label: string) => {
    ...
    return { count: 0, tone: "blue" as const };
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto pb-6 px-2 no-scrollbar">
        <Files
          ...
          className="w-full"
          style={{ overflow: 'visible' }} // Override hardcoded auto to prevent scrollbars
        >
          <FilesHighlight className="bg-accent rounded-lg pointer-events-none" click={false}>
            {visibleGroups.map((group) => {
              const GroupIcon = group.icon;
              return (
                <FolderItem key={group.groupKey} value={group.groupKey}>
                  {/* Custom folder header  no folder icon, uses semantic GroupIcon */}
                  <FolderHeader>
                    <FolderTrigger className="w-full text-start">
                      <FolderHighlight click={false}>
            ...
            setDialogEntityId(""); // Empty entityId means show all for this type
