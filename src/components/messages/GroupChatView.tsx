...
export default function GroupChatView({ group, currentUserId }: GroupChatViewProps) {
  ...
  useEffect(() => {
    // Mark-as-read should happen when opening the group, not on every message update.
    markGroupMessagesAsRead(group.id).then(() => router.refresh());
  }, [group.id, router]);
  ...
  }, [group.messages.length, isNearBottom]);
  ...
  const handleSendMessage = async (content: string) => {
    ...
    setIsSending(true);
    try {
      await sendGroupMessage(group.id, content, replyToMessage?.id);
    ...
    <div className="flex flex-col h-full bg-background relative overflow-hidden">
      ...
      >
        {group.messages.map((msg: any) => {
          ...
          return (
            <div key={msg.id} className={`flex gap-3 ${isMe ? "flex-row-reverse self-end" : "flex-row self-start"} w-full max-w-[85%]`}>
              {!isMe && (
