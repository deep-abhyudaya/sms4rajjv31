...
export default function DirectMessageClient({
  ...
  }, [selectedData?.messages?.length, isNearBottom]);

  useEffect(() => {
    if (selectedData) {
      if (selectedData.isGroup) {
        markGroupMessagesAsRead(selectedData.id).then(() => router.refresh());
      } else {
        markDirectMessagesAsRead(selectedData.id).then(() => router.refresh());
      }
    }
  ...
  const handleSendMessage = async (content: string) => {
    ...
    await sendDirectMessage(selectedData.id, content, replyToMessage?.id);
            ...
            selectedData.isGroup ? (
              ...
              <div className="flex flex-col h-full">
                ...
                >
                  ...
                  {selectedData.messages.map((msg: any) => {
