...
export async function getConversations() {
  ...
  const enrichedConversations = await Promise.all(
    conversations.map(async (conv) => {
      ...
      let otherUser: any = await prisma.student.findUnique({ where: { id: otherId }, select: { username: true, name: true, surname: true } });
      let role = "student";

      if (!otherUser) {
        otherUser = await prisma.teacher.findUnique({ where: { id: otherId }, select: { username: true, name: true, surname: true } });
        role = "teacher";
      }
      if (!otherUser) {
        otherUser = await prisma.parent.findUnique({ where: { id: otherId }, select: { username: true, name: true, surname: true } });
        role = "parent";
      }
      if (!otherUser) {
        otherUser = await prisma.admin.findUnique({ where: { id: otherId }, select: { username: true } });
        role = "admin";
        if (otherUser) {
